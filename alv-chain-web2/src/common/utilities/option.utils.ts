...
const unwrapOrThrow = <T>(opt: OptionType<T>, throwWith: unknown): T => {
  if (opt._type !== "some") throw throwWith;
  return opt.value;
};
...
export class Optional<T> {
  ...
  unwrapOrThrow = (throwWith: unknown) =>
    OptionUtils.unwrapOrThrow(this.option, throwWith);
}
